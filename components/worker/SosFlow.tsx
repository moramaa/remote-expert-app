...
import {
  ...
  XCircle, Search, X, Loader2, Users,
} from 'lucide-react';
        ...
        >
          ...
          <input
            ...
            style={{
              ...
              background: 'transparent', fontSize: '13px', color: '#0F172A',
            }}
          />
          {query && (
            <button
...
function SuggestionsStep({
  ...
  const [answered, setAnswered]         = useState<string | null>(null); // sessionId being answered
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: '13px', color: '#64748B', fontWeight: 500 }}>
              No past solutions found for this machine.
            </p>
            <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#94A3B8' }}>
              Connect directly to a live expert below.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setExpanded(isExpanded ? null : s.sessionId);
                      setAnswered(s.sessionId);
                    }}
                    style={{
                      display: 'flex',
                      alignItems: 'flex-start',
                  ...
                  {/* Expanded: "Did this solve your issue?" */}
      ...
      {/* Escalation CTA — always pinned at bottom */}
      <div
        style={{
      ...
      >
        <p style={{ margin: '0 0 10px', fontSize: '11px', color: '#94A3B8', textAlign: 'center' }}>
          {suggestions.length > 0
            ? 'None of these solved your problem?'
            : 'Need immediate help?'}
        </p>
        <button
          type="button"
    ...
    >
        ...
        {[0, 0.5].map((delay) => (
          <div
            ...
            style={{
              ...
              animation: `sos-ping 1.5s cubic-bezier(0,0,0.2,1) ${delay}s infinite`,
              opacity: 0.4,
            }}
          />
        ))}
      ...
      <div>
        ...
        <div style={{ fontSize: '13px', color: '#64748B', marginTop: '6px' }}>
          Searching for a certified expert in
        </div>
        <div style={{ fontSize: '15px', fontWeight: 600, color: '#1D4ED8', marginTop: '4px' }}>
          {machineName}
        </div>
...
export default function SosFlow({ profile, socket, isConnected, onSessionJoined, onClose }: Props) {
  ...
  // Listen for session:join / ticket-status while searching
  useEffect(() => {
    if (!socket || step !== 'searching') return;

    const onTicketStatus = (payload: {
      ticketId: string; state: string; sessionId?: string;
    }): void => {
  ...
  const startLiveSearch = useCallback(() => {
    ...
    socket.emit(
      'worker:sos-create',
      { machineId, workerName: profile.name, workerFactory: profile.factory },
      (result: SosAck) => {
        if ('error' in result) {
          setStep('suggestions'); // roll back
        } else {
          setTicketId(result.ticketId);
        }
      },
    );
  }, [socket, isConnected, machineId, profile]);
